# Configuración de Firewalld

**Idiomas:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/es/courses/firewalld-configuration">
    <img src="https://course-cover.labex.io/firewalld-configuration.png?lang=es" alt="Configuración de Firewalld">
  </a>
</p>

Aprenda la Configuración de Firewalld. Este módulo cubre los Fundamentos y Zonas de Firewalld, Gestión de Servicios y Puertos, Reglas Enriquecidas (Rich Rules), Reenvío de Puertos (Port Forwarding), y Modo Pánico (Panic Mode). Dominará estas habilidades esenciales de Linux a través de laboratorios prácticos y desafíos del mundo real.

[Iniciar curso en LabEx](https://labex.io/es/courses/firewalld-configuration)

## Ejercicios

|   Índice | Nombre                               | Dificultad   | Práctica                                                                                                                             |
|----------|--------------------------------------|--------------|--------------------------------------------------------------------------------------------------------------------------------------|
|        1 | Fundamentos de Firewalld y Zonas     | Principiante | <a target='_blank' href='https://labex.io/es/labs/firewalld-basics-and-zones-636553?course=firewalld-configuration'>Iniciar lab</a>  |
|        2 | Gestión de servicios y puertos       | Principiante | <a target='_blank' href='https://labex.io/es/labs/managing-services-and-ports-636554?course=firewalld-configuration'>Iniciar lab</a> |
|        3 | Reglas enriquecidas (Rich Rules)     | Principiante | <a target='_blank' href='https://labex.io/es/labs/rich-rules-636557?course=firewalld-configuration'>Iniciar lab</a>                  |
|        4 | Reenvío de puertos (Port Forwarding) | Principiante | <a target='_blank' href='https://labex.io/es/labs/port-forwarding-636556?course=firewalld-configuration'>Iniciar lab</a>             |
|        5 | Modo de pánico                       | Principiante | <a target='_blank' href='https://labex.io/es/labs/panic-mode-636555?course=firewalld-configuration'>Iniciar lab</a>                  |
|        6 | Servidor Web Reforzado               | Intermedio   | <a target='_blank' href='https://labex.io/es/labs/hardened-web-server-636739?course=firewalld-configuration'>Iniciar desafío</a>     |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

